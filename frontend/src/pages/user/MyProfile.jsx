...
const MyProfile = () => {
  ...
  const handleToggleEdit = () => {
    if (isEditing) {
      // If canceling, reset to original user data
      setProfile({
        name: user.name || "",
        email: user.email || "",
        mobile: user.mobile || "",
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden transition-all duration-300">
          ...
          <div className="p-8 md:p-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              {/* Keep address field but in minimal style if present */}
              <div className="space-y-2 md:col-span-2">
                <label className="text-xs font-bold text-slate-400 ml-1">
                  {t("complaints.profile.address")}
                </label>
                <textarea
                  name="address"
                  className={`w-full px-4 py-3 bg-[#f8fafc] dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 text-sm font-medium text-slate-700 transition-all outline-none min-h-[100px] ${!isEditing ? 'opacity-70 cursor-not-allowed' : ''}`}
                  value={profile.address}
                  onChange={handleChange}
                  disabled={!isEditing}
                  placeholder={t("complaints.profile.address_placeholder")}
                />
              </div>
            </div>
          </div>
